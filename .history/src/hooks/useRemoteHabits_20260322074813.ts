...
export function useRemoteHabits({ getToday }: UseRemoteHabitsOptions = {}) {
  ...
  useEffect(() => {
    const loadFromDB = async () => {
      if (isAnonymous) {
        // Use last viewed data for anonymous users (after logout)
        setHabits(load(LAST_VIEWED_HABITS_KEY, []));
        setEntries(load(LAST_VIEWED_ENTRIES_KEY, []));
        setPlanetObjects(load(LAST_VIEWED_PLANET_KEY, []));
        setLoading(false);
        return;
      }
